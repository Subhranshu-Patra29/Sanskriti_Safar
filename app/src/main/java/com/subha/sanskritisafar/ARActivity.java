...
public class ARActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        // Get which is the top recommendation
        selectedLanguage = getIntent().getStringExtra("SELECTED_LANGUAGE");
        heritageSiteID = getIntent().getStringExtra("heritage_site_id");
    ...
    private void retrieveRecommendationData() {
        Log.e("PRINTOUT", heritageSiteID);

        // Firebase database reference
        DatabaseReference databaseRef = FirebaseDatabase.getInstance().getReference("recommendations");

        databaseRef.orderByChild("id").equalTo(heritageSiteID).addListenerForSingleValueEvent(new ValueEventListener() {
            @Override
            public void onDataChange(@NonNull DataSnapshot dataSnapshot) {
                for (DataSnapshot snapshot : dataSnapshot.getChildren()) {
                    // Parse the retrieved recommendation
                    Recommendation recommendation = snapshot.getValue(Recommendation.class);

                    // Use the recommendation data
                    if (recommendation != null) {
                        heritageSiteName = recommendation.getTitle();
                        short_description = recommendation.getShort_description();
                        long_description = recommendation.getLong_description();
                        imageURL = recommendation.getImageUrl();
                        imageUrl360 = recommendation.getImageUrl360();
                        objects = recommendation.getObjects();
                        ...
                        // Middle: Setup 360-degree view
                        setup360View();

                        // Update TabAdapter with long description
//                        ViewPager2 viewPager = findViewById(R.id.viewPager);
//                        viewPager.setAdapter(new TabAdapter(ARActivity.this, long_description));

                        TabAdapter tabAdapter = new TabAdapter(ARActivity.this, long_description, insideObjects, outsideObjects);
                        ViewPager2 viewPager = findViewById(R.id.viewPager);
